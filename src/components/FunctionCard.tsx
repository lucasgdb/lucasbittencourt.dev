...
export default function FunctionCard({ title, description, slug, logo, ...rest }) {
  return (
    <Link
      href={`/snippets/${slug}`}
      ...
      {...rest}
    >
      <Image alt={title} height={32} width={32} src={urlForImage(logo).url()} className="rounded-full" />
